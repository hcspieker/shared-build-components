...
```yaml
    - name: Build dotnet projects
      uses: hcspieker/shared-build-components/.github/actions/dotnet-build@v3.1.1
      with:
...
jobs:
  call-dotnet_web_build:
    uses: hcspieker/shared-build-components/.github/workflows/dotnet_web_build.yml@v3.1.1
    with:
...
- For production workflows pin to a tag or commit SHA, e.g. `uses: hcspieker/shared-build-components/.github/actions/dotnet-build@v3.1.1` or `@<sha>`.
